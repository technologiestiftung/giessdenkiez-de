...
const Linechart = p => {
  ...
  // TODO: function init creates multiple errors
  // eslint says it should be moved inside useCallback or inti the hook
  // if in the hook the variables width, and so on create errors
  // mavbe they shoud be setState
  const init = data => {
    const wrapper = select('#linechart');
    width = wrapper.node().clientWidth;
    height = wrapper.node().clientHeight;

    svg = wrapper.append('svg').attr('width', width).attr('height', height);
