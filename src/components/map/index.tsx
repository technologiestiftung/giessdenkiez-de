...
interface DeckGLPropType {
  ...
  visibleLayer: StoreProps['visibleLayer'];
...
class DeckGLMap extends React.Component<DeckGLPropType, DeckGLStateType> {
  ...
  _renderLayers(): unknown[] {
    const { data, rainGeojson, visibleLayer, pumps } = this.props;
    ...
    const layers = [
      new GeoJsonLayer({
        ...
        visible: visibleLayer === 'trees' ? true : false,
        filled: true,
        parameters: {
          depthTest: false,
        },
      ...
      new GeoJsonLayer({
        ...
        data: rainGeojson,
        opacity: 0.95,
        visible: visibleLayer === 'rain' ? true : false,
        stroked: false,
        filled: true,
        extruded: true,
        wireframe: true,
      ...
      new GeoJsonLayer({
        ...
        data: pumps,
        opacity: 1,
        visible: visibleLayer === 'pumps' ? true : false,
        stroked: true,
        filled: true,
        extruded: true,
        wireframe: true,
  ...
  _updateStyles(prevProps: DeckGLPropType): void {
    if (map && isMobile) {
      ...
      if (this.props.visibleLayer !== 'trees') {
